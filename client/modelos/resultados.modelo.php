...
class ModeloResultados
{
	...
	static public function mdlMisQuinielas($id_usuario)
	{
		...
		INNER JOIN jornadas AS j ON j.id = q.id_jornada AND q.id_acceso = :idacceso  WHERE DATE_ADD(q.fecha_registro, 
	...
	static public function mdlQuinielasxJornada($id_usuario,$id_joranda)
	{
		...
		WHERE id = q.id_ligas) nomLigas, q.fecha_registro, DATE_ADD(q.fecha_registro,INTERVAL 3 DAY) AS diasAtras
		FROM quinielas AS q	INNER JOIN jornadas AS j ON j.id = q.id_jornada AND q.id_acceso = :idacceso
		AND j.id = :id_joranda WHERE DATE_ADD(q.fecha_registro, INTERVAL 10 DAY) > CURRENT_TIMESTAMP()
		AND j.estado = 'A' 	OR j.estado = 'D'
	GROUP BY q.id ORDER BY q.fecha_registro DESC");
		$conexion->bindParam("idacceso", $id_usuario, PDO::PARAM_INT);
	...
	static public function mdlgetEncuentros($id_jornada)
	{
		...
		DATE_FORMAT(e.fecha_p,'%r') as hora_partido,e.resultado FROM encuentros AS e WHERE e.id_jornada  = :id_jornada;");
		$conexion->bindParam("id_jornada", $id_jornada, PDO::PARAM_INT);
		if ($conexion->execute()) {
			return $conexion->fetchAll();
		}
