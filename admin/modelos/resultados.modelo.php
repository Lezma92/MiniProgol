...
class ModeloResultados
{
	...
	static public function mdlQuinielasxJornada($id_usuario, $id_joranda)
	{
		...
		AS diasAtras FROM quinielas AS q INNER JOIN	jornadas AS j ON j.id = q.id_jornada AND
		q.id_acceso = :idacceso AND j.id = :id_joranda	WHERE
		DATE_ADD(q.fecha_registro,INTERVAL 10 DAY) > CURRENT_TIMESTAMP()
